...
export const fetchSingleArtworkMetApi = async (
  id: string,
): Promise<Artwork[]> => {
  try {
    const response = await axios.get<Artwork[]>(
      "/.netlify/functions/fetchMetSingleArtworkDetails",
      {
        params: { id },
      }
    );
    ...
    const artworks = responseData.map((object: any) => {
      ...
      const artist_nationality =
        object.artistNationality || "Unknown nationality";
      ...
      const classification =
        object.classification || "an extraordinary artwork";
      const department = object.department || "an unspecified department";
      const objectUrl =
        object.objectURL || "https://www.metmuseum.org/art/collection";
